...
impl TDigest {
    ...
    /// Along with the [`Self::centroids()`], [`Self::sum()`],
    /// [`Self::max_size()`], [`Self::count()`], [`Self::max()`], and
    /// [`Self::min()`] accessors, it lets a digest be serialized into and
    /// restored from a caller's own format without round-tripping through a
    /// [`ScalarValue`].
    ///
    /// Unlike [`Self::from_scalar_state()`], this method validates its inputs:
    /// it may be handed state decoded from an external persister's format, so
    /// it rejects the invariant violations listed below with an error instead
    /// of producing a silently wrong digest. Callers who trust their data can
    /// `unwrap()`.
    ///
    /// # Errors
    ///
